...
TRANSLATIONS = {
    "pt": {
        ...
        "colored_matrix": "Matriz de avaliação",
...
def analysis_export_tables(analysis) -> dict[str, pd.DataFrame]:
    tables: dict[str, pd.DataFrame] = {
        ...
        "Matriz de avaliação": evaluation_matrix_dataframe(analysis),
    }
...
def build_complete_excel(analysis) -> bytes:
    ...
    level_colors = {level.code: level.color for level in scale_for_analysis(analysis)}
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        matrix_sheet = writer.sheets.get("Matriz de avaliação")
        if matrix_sheet is not None:
            for row in matrix_sheet.iter_rows(min_row=2, min_col=5):
                for cell in row:
                    level = str(cell.value or "N").split(" ", 1)[0]
                    color = _clean_color(level_colors.get(level, "#EAF4EC")).replace("#", "")
...
def build_complete_pdf(analysis) -> bytes:
    ...
    for name, table in analysis_export_tables(analysis).items():
        ...
        is_matrix = name == "Matriz de avaliação"
        printable = table.copy().astype(str)
        truncated = False
        if is_matrix and len(printable.columns) > 16:
            printable = printable.iloc[:, list(range(4)) + list(range(4, 16))]
            truncated = True
        max_rows = 28 if not is_matrix else 18
        ...
        if is_matrix:
            ...
            for row_index, row in enumerate(data[1:], start=1):
                for column_index, value in enumerate(row[4:], start=4):
                    level = str(value or "N").split(" ", 1)[0]
                    color = level_colors.get(level)
                    if not color:
                        continue
                    table_style.append(("BACKGROUND", (column_index, row_index), (column_index, row_index), colors.HexColor(_clean_color(color))))
                    text_color = colors.white if text_color_for_background(color) == "white" else colors.HexColor(DARK_BLUE)
                    table_style.append(("TEXTCOLOR", (column_index, row_index), (column_index, row_index), text_color))
                    table_style.append(("FONTNAME", (column_index, row_index), (column_index, row_index), "Helvetica-Bold"))
